...
data2 <- df2 %>% 
  select(ID, Frequency, Media, SAT, SATdetail, SATabstract, Age, Gender) 
...
model1 <- lm(SAT ~ Media, data = df)
model3 <- lm(SATdetail ~ Media, data = df)
model4 <- lm(SATabstract ~ Media, data = df)

...
df1 <- read.csv(here("iPadProjectData_ElsaEdit.csv"))
df2 <- read.csv(here("iPadExperiment.csv"))
```
...
df1 <- df1 %>% 
  mutate(BIF = rowSums(df1[,c('BIF1','BIF2','BIF3','BIF4','BIF5',
...
data2 <- df2 %>% 
  select(ID, Frequency, Media, SAT, SATdetail, SATabstract, Age) 

df <- left_join(data2, data1, by = "ID")
```
...
model1 <- lm(SAT ~ Media + Frequency, data = df)
model2 <- lm(BIF ~ Media + Frequency, data = df)
model3 <- lm(SATdetail ~ Media + Frequency, data = df)
model4 <- lm(SATabstract ~ Media + Frequency, data = df)
...
grouped <- df %>% 
    ...
    summarise(
    count = n(),
    mean = mean(SATabstract, na.rm = TRUE),
    sd = sd(SATabstract, na.rm = TRUE)
)
grouped$mean
grouped$sd
```
